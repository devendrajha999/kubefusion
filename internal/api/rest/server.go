...
func (s *Server) Register(r *gin.Engine) { /* unchanged */
	...
	a.Use(requireAuth(s.JWTSecret))
	{
		...
		a.GET("/clusters/:name/namespaces", s.listNamespaces)
		...
		a.GET("/clusters/:name/pods/:namespace/:pod", s.podDetails)
		a.DELETE("/clusters/:name/pods/:namespace/:pod", requireRole("admin", "operator"), s.deletePod)
...
func (s *Server) listNamespaces(c *gin.Context) { if s.Kube==nil { c.JSON(http.StatusOK, []gin.H{{"name":"default","status":"Active"}}); return }; nss,err:=s.Kube.ListNamespaces(c.Request.Context()); if err!=nil { c.JSON(http.StatusBadGateway, gin.H{"error":err.Error()}); return }; c.JSON(http.StatusOK,nss) }
...
func (s *Server) listDeployments(c *gin.Context) { ns:=c.Query("namespace"); if s.Kube==nil { c.JSON(http.StatusOK, []gin.H{{"namespace":"default","name":"example","replicas":1,"ready":1,"updated":1}}); return }; deps,err:=s.Kube.ListDeployments(c.Request.Context(),ns); if err!=nil { c.JSON(http.StatusBadGateway, gin.H{"error":err.Error()}); return }; c.JSON(http.StatusOK,deps) }
func (s *Server) listResourceByKind(c *gin.Context) { ns:=c.Query("namespace"); kind:=c.Param("kind"); if s.Kube==nil { c.JSON(http.StatusOK, []gin.H{}); return }; items,err:=s.Kube.ListResource(c.Request.Context(), kind, ns); if err!=nil { c.JSON(http.StatusBadRequest, gin.H{"error":err.Error()}); return }; c.JSON(http.StatusOK,items) }
...
func (s *Server) podDetails(c *gin.Context) { if s.Kube==nil { c.JSON(http.StatusNotImplemented, gin.H{"error":"kubernetes client unavailable"}); return }; ns:=c.Param("namespace"); pod:=c.Param("pod"); d,err:=s.Kube.PodDetails(c.Request.Context(),ns,pod); if err!=nil { c.JSON(http.StatusBadGateway, gin.H{"error":err.Error()}); return }; c.JSON(http.StatusOK,d) }
func (s *Server) deletePod(c *gin.Context) { if s.Kube==nil { c.JSON(http.StatusNotImplemented, gin.H{"error":"kubernetes client unavailable"}); return }; ns:=c.Param("namespace"); pod:=c.Param("pod"); if err:=s.Kube.DeletePod(c.Request.Context(),ns,pod); err!=nil { c.JSON(http.StatusBadGateway, gin.H{"error":err.Error()}); return }; s.audit(c,"pod.delete",ns+"/"+pod); c.JSON(http.StatusOK, gin.H{"status":"deleted"}) }
